...
const globalvar_tmp_path = "tmp/global_info.json";
...
async function autoSendResultEmail(resultFolder) {
    ...
    try {
        ...
        if (!fs.existsSync(parameters_path)) {
            console.log("[email_handler][WARN] parameters.json not found");
            return;
        }

        const params = JSON.parse(fs.readFileSync(parameters_path, "utf8"));
