...
document.addEventListener("DOMContentLoaded", () => {
  ...
  socket.on("jsonData", (data) => {
    ...
    for (let i = 1; i <= 5; i++) {
      ...
      if (stageData.voltage?.enabled === false) {
        ...
        toggleBtn.textContent = t("BUTTON_TURNON");
        ...
        toggleBtn.textContent = t("BUTTON_TURNOFF");
  ...
  for (let i = 2; i <= MAX_STAGE; i++) {
    ...
    clone.querySelector(".stage-title").innerText = t("LABEL_STAGE", { n: i });
  ...
  function initToggle(btn) {
    ...
    btn.addEventListener("click", () => {
      ...
      if (btn.dataset.state === "off") {
        ...
        btn.innerText = t("BUTTON_TURNOFF");
        input.disabled = false;
      } else {
        btn.dataset.state = "off";
        btn.innerText = t("BUTTON_TURNON");
        input.value = 0;
        input.disabled = true;
      }
    });
  ...
  btnNext.addEventListener("click", () => {
    ...
    if (automail) {
      const ask = confirm(t("ALERT_AUTOMAIL"));
      if (!ask) {
        goToPage("./Electrophoresis/electrophoresis.html");
        return;
      }
